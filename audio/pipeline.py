...
# Import local modules (audio directory is added to sys.path by callers)
import transcribe
...
def run_pipeline(input_video_path, output_dir=None, artifacts_dir=None):
    ...
    artifacts_dir = artifacts_dir or os.path.join(output_dir, "pipeline_artifacts")
    ...
    analysis_path = os.path.join(artifacts_dir, "analysis.json")
    try:
    ...
    final_output = os.path.join(output_dir, "edited_video.mp4")
    ...
    return {
        ...
        "transcript_json": json_path,
        "analysis_path": analysis_path,
        "replacements": [rep.dict() for rep in analysis.replacements],
        "artifacts_dir": artifacts_dir,
    }
